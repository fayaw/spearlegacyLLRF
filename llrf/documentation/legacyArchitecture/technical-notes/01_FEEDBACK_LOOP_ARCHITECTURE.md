...
| 7 | **Gap Voltage Feed-Forward (GVF)** | Provides IQ reference values for gap voltage setpoint and interfaces with Longitudinal Feedback (LFB) system for low-order coupled-bunch damping. *Feed-forward path, not a feedback loop.* | N/A (feed-forward) | VXI GVF module; fiber optic TAXI link to LFB | ❌ PEP-II only |
