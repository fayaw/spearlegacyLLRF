...
\begin{figure}[p]
\centering
\begin{fitpicture}\input{tikz/fig-upgraded-architecture}\end{fitpicture}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-rf-signal-flow}\end{fitpicture}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-hvps-power-chain}\end{fitpicture}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-arc-detection}\end{fitpicture}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-heater-architecture}\end{fitpicture}
...
The matrix above uses six physically distinct interface types. The division matters because it is what makes the "no safety function depends on the network" claim in Section~\ref{non-epics-communication-paths} true by construction.
...
Dry contact / solid-state relay & Arc detector outputs & Arc receivers $\rightarrow$ Arc Detection Chassis & Native output of the Microstep-MIS receivers; contact closure needs no supply on the sensor side \\
Analog & Heater voltage programming; HVPS voltage and current; V/I metering & MPS $\rightarrow$ Chroma; HVPS $\rightarrow$ Waveform Buffer and MPS & Continuous quantity with a hardware path that stays valid if the network is down \\
...
The following interface details are not yet settled and are tracked here rather than being buried in the sections above. Each must be closed before the corresponding subsystem can be integrated.

\begin{longtable}[]{@{}P{\dimexpr 0.26\linewidth-1.5\tabcolsep\relax}P{\dimexpr 0.51\linewidth-1.5\tabcolsep\relax}P{\dimexpr 0.23\linewidth-1.5\tabcolsep\relax}@{}}
\toprule\noalign{}
\textbf{Interface} & \textbf{Open question} & \textbf{Needed before} \\
\midrule\noalign{}
\endhead
LLRF9 $\leftrightarrow$ Interface Chassis & Exact voltage logic levels for the interlock input and output. The input is selectable (3.3/5/24\,V) and the output is specified as \qty{4.75}{\volt} high through \qty{220}{\ohm}; the Interface Chassis input and output stages must be designed to match. & Interface Chassis PCB design \\
EPICS version & The LLRF9 internal IOC runs EPICS Base 3.14, older than the SSRL site standard. Whether the units are migrated or bridged by a gateway is undecided. & Coordinator software design \\
LLRF9 FPGA & Board generation (Spartan-6 vs.\ Artix-7) to be confirmed with the vendor; affects available firmware features. & Firmware feature commitments \\
Network addressing & IP addresses and subnet allocation for both LLRF9 units, both PLCs, the motion controller and the Waveform Buffer are all TBD. & Any integration testing \\
Soft IOC location & Whether the coordinator IOC is hosted in B132 or B137 is not yet confirmed with the controls group. & Rack and network planning \\
Heater control & The ASYN/VISA EPICS module for the Chroma supply does not exist yet; the ASCII command subset to be supported is not defined. & Heater subsystem commissioning \\
Tuner control split & Whether the tuner loop is closed inside the LLRF9 or in the coordinator using LLRF9 phase data (Section~\ref{tuner-control-loops-4-1-hz}). & Motion controller acceptance \\
PPS connector & The installed connector part number requires field verification; the original Burndy circular 8-pin may already have been replaced by the Souriau Trim Trio equivalent. & PPS Interface Box fabrication \\
Waveform Buffer spares & Two RF channels are unassigned. One is a candidate for a coupled klystron forward power signal feeding the collector power calculation. & Waveform Buffer PCB layout \\
\bottomrule\noalign{}
\end{longtable}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-fault-propagation}\end{fitpicture}
