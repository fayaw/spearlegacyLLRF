...
\section*{Document Scope and Provenance}\label{document-scope-and-provenance}
...
All technical content is derived from \textbf{original source documents} per the Documentation Architecture Proposal (v5.1, §2.1). AI-generated technical notes are cited as \emph{"preliminary analysis (AI-generated, see {[}filename{]}, unreviewed)"} per §2.4.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{System Overview and Field Stability Requirements}\label{system-overview-and-field-stability-requirements}


\subsection{The Control Problem in One Sentence}\label{the-control-problem-in-one-sentence}
...
\subsection{Storage Ring Parameters}\label{storage-ring-parameters}
...
\begin{quote}
\textbf{Note on \(V_\text{RF}\)}: The \textbf{design} value of 3.2 MV (800 kV/cavity) is from the original SPEAR3 RF system specification {[}R1{]}. The \textbf{operational} value of \(\sim 2.85\) MV (712 kV/cavity) is from LLRF9 commissioning measurements {[}R4{]} (also the current operation norminal at 500 mA ) and is used throughout the beam loading calculations in this document.
\end{quote}

\begin{quote}
\textbf{Note on \(\alpha_c\)}: Published values range from 0.0011 (SSRL parameter page {[}R3{]}) to 0.00118 (used in this document) depending on the specific lattice optics. Differences of \(\sim 7\%\) in \(\alpha_c\) produce \(\sim 3\%\) differences in \(f_s\).
\end{quote}

\begin{quote}
\textbf{Sources}: {[}R1{]} McIntosh et al., SLAC-PUB-10983, EPAC 2004 (Table 1: \(Q_s = 0.008\), \(\alpha = 0.00113\)). {[}R2{]} Hettel et al., PAC 1999. {[}R3{]} SSRL SPEAR Storage Ring Parameters (\(\nu_s = 0.007\), \(\alpha_c = 0.0011\), \(V_\text{RF} = 3.2\) MV). {[}R5{]} \texttt{Designs/\allowbreak{}tex/\allowbreak{}doc0-\allowbreak{}system-\allowbreak{}design/\allowbreak{}0\_\allowbreak{}system\_\allowbreak{}design\_\allowbreak{}report.\allowbreak{}pdf}.
\end{quote}

\subsection{RF System Configuration}\label{rf-system-configuration}
...
\begin{quote}
\textbf{Sources}: {[}R1{]}; {[}R6{]} Schwarz, PS-340-330-51-R0, 1999.
\end{quote}

\subsection{Field Stability Requirements}\label{field-stability-requirements}
...
\begin{quote}
\textbf{Sources}: {[}R5{]}; {[}R4{]} LLRF9 Commissioning Tests.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Plant Physics Model}\label{plant-physics-model}
...
\subsection{High-Level System Block Diagram}\label{high-level-system-block-diagram}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-plant-overview}\end{fitpicture}
...
\subsection{RF Cavity as a Narrowband Resonator}\label{rf-cavity-as-a-narrowband-resonator}


\subsubsection{Equivalent Circuit}\label{equivalent-circuit}
...
\subsubsection{Beam Loading --- Steady-State Phasor Analysis}\label{beam-loading--steady-state-phasor-analysis}
...
This exceeds the desired gap voltage (\(V_\text{gap} = 712\) kV) by a factor of 2.6, demonstrating that \textbf{beam loading is the dominant effect}.
...
\textbf{Optimum detuning} --- minimizes reflected power at the input coupler:
...
The cavity must be tuned \(\sim 87\) kHz \textbf{below} \(f_\text{RF}\) at 500 mA.
...
\textbf{Total RF power}: \(4 \times 196 = 782\) kW --- within the 1.2 MW klystron capacity (\(\sim 35\%\) margin). The measured forward power at 500 mA is \(\sim 200\) kW/cavity, consistent with this calculation (the small excess accounts for reflected power at non-ideal coupling).

\begin{quote}
\textbf{Note}: Eq. 2.7 is the minimum-power condition at optimum coupling. In the general case with arbitrary coupling \(\beta\) and detuning \(\psi\), the generator power includes reflected power terms. The important identity is that beam power per cavity \(= I_b V_\text{gap}\cos\phi_s = I_b \, U_0/n_\text{cav}\).
\end{quote}

\begin{quote}
\textbf{Sources}: {[}R11{]} Gamp, CAS 2011; {[}R12{]} Wilson, SLAC-PUB-6062; {[}R13{]} Boussard, PAC 1985.
\end{quote}

\subsection{Klystron as the Actuator}\label{klystron-as-the-actuator}
...
\textbf{AM-PM conversion} --- klystron output phase sensitivity to cathode voltage:
...
\begin{quote}
\textbf{Sources}: {[}R1{]}; {[}R15{]} Corredoura, PAC 1999; {[}R5{]}.
\end{quote}

\subsection{Loop Delay Budget --- The Fundamental Bandwidth Ceiling}\label{loop-delay-budget--the-fundamental-bandwidth-ceiling}
...
\textbf{Note}: Diret loop delay is 270 ns from LLRF9 manual, which is assumed the total delay of the loop as listed here {[}To be confirmed{]}.

\subsection{Complete Open-Loop Plant Transfer Function}\label{complete-open-loop-plant-transfer-function}
...
\section{I/Q Signal Processing Framework}\label{iq-signal-processing-framework}


\subsection{Baseband I/Q Representation}\label{baseband-iq-representation}
...
\subsection{Baseband I/Q Modulator}\label{baseband-iq-modulator}
...
\textbf{Implementation}: Each baseband I/Q modulator uses four AD834 four-quadrant multipliers (DC to 500 MHz) + two EL2073 summing amplifiers to implement the 2×2 matrix. Group delay \(< 5\) ns, full-power BW \(> 40\) MHz, dynamic range \(> 50\) dB.
...
\begin{quote}
\textbf{Sources}: {[}R15{]} Corredoura, Eq. 2 (PEP-II configuration); {[}R14{]} Schwarz, PS-340-330-52-R0; Doc B §13.2 (SPEAR3 RFP module description).
\end{quote}

\subsection{I/Q Demodulation}\label{iq-demodulation}
...
\subsection{Cavity Probe Vector Sum}\label{cavity-probe-vector-sum}
...
\subsection{Error Signal Generation}\label{error-signal-generation}
...
\section{Disturbance Analysis and Control Problem Statement}\label{disturbance-analysis-and-control-problem-statement}


\textbf{The control architecture follows inevitably from the disturbance landscape.}

\subsection{Disturbance Taxonomy}\label{disturbance-taxonomy}
...
\subsection{D1/D2: Beam Loading --- The Dominant Disturbance}\label{d1d2-beam-loading--the-dominant-disturbance}
...
\textbf{How feedback suppresses D1/D2}: The direct feedback loop reduces the effective impedance seen by the beam:
...
\item
  \textbf{Stability}: The beam-cavity interaction becomes stable because the reduced impedance eliminates the conditions for exponential growth. The \textbf{growth rate} for coupled-bunch instability from the fundamental mode {[}R15{]}:
\end{enumerate}
...
\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-beam-loading-phasor}\end{fitpicture}
...
\begin{quote}
\textbf{Sources}: {[}R15{]} Corredoura, PAC 1999, Eq. 1; {[}R11{]} Gamp, CAS 2011.
\end{quote}

\subsection{D3: Robinson Instability}\label{d3-robinson-instability}
...
\begin{quote}
\textbf{Sources}: {[}R16{]} Robinson, CEAL-1010; {[}R11{]} Gamp; {[}R13{]} Boussard.
\end{quote}

\subsection{D4: Coupled-Bunch Modes}\label{d4-coupled-bunch-modes}
...
\subsection{D5: HVPS Ripple}\label{d5-hvps-ripple}
...
The 360 Hz component (first 6-pulse harmonic absent in ideal 12-pulse operation) is the \textbf{dominant residual} and the primary target of the ripple loop. Additional low-order harmonics at 60, 120, 180, 240 Hz may appear at reduced levels (\(\sim 20\)--\(40\) dB below 360 Hz) depending on SCR firing symmetry. Coupling to RF field via AM-PM conversion:
...
\begin{quote}
\textbf{Sources}: {[}R21{]}; {[}R22{]}; \fpath{llrf/documentation/legacyArchitecture/feedbackLoopDescriptionps3403305200.pdf}.
\end{quote}

\subsection{D6: Klystron Gain Variation}\label{d6-klystron-gain-variation}
...
\begin{quote}
\textbf{Sources}: {[}R14{]} Schwarz, PS-340-330-52-R0; {[}R15{]} Corredoura; Doc B §7.6.
\end{quote}

\subsection{D7/D8: Microphonics and Thermal Detuning}\label{d7d8-microphonics-and-thermal-detuning}
...
\begin{quote}
\textbf{Note}: This first-principles estimate (\(-7.9\) kHz/°C) applies to the uncooled cavity body. In practice, the PEP-II-type cavities at SPEAR3 are water-cooled with a regulated cooling circuit that maintains body temperature to within \(\sim \pm 0.1\)°C.
\end{quote}
...
\subsection{D9: Ring Circumference Thermal Drift --- Long-Term RF Frequency Migration}\label{d9-ring-circumference-thermal-drift--long-term-rf-frequency-migration}
...
\textbf{Measured magnitude at SPEAR3}: The RF operating frequency drifts by approximately \(\Delta f_\text{RF} \approx 4\) kHz over one year. From the harmonic condition:
...
\textbf{Rate} & \(-7.9\) kHz/°C (Eq. 4.7); & \(\sim 4\) kHz/year (measured operational data) \\
...
\subsection{Frequency-Domain Summary: The Disturbance Spectrum}\label{frequency-domain-summary-the-disturbance-spectrum}


\begin{figure}[tbp]
\centering
\begin{fitpicture}\input{tikz/fig-disturbance-spectrum}\end{fitpicture}
...
\textbf{This table is the Rosetta Stone of the control design.} Each loop exists because one or more disturbances occupy a specific frequency band.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Control Architecture: From Disturbances to Loops}\label{control-architecture-from-disturbances-to-loops}


\subsection{The Design Logic: Bandwidth Matching}\label{the-design-logic-bandwidth-matching}
...
\subsection{Direct Feedback Loop --- Compensating Beam Loading (D1--D4)}\label{direct-feedback-loop--compensating-beam-loading-d1d4}
...
This is the central result. The direct loop \textbf{transforms the cavity from a high-impedance resonator into a low-impedance broadband structure}:
...
\begin{quote}
\textbf{Sources}: {[}R15{]} Corredoura, Fig. 3; {[}R14{]}; {[}R11{]}.
\end{quote}

\subsection{Comb Loop --- Narrowband Enhancement at Revolution Harmonics (D4)}\label{comb-loop--narrowband-enhancement-at-revolution-harmonics-d4}


\textbf{Z-domain transfer function} {[}R15{]}:
...
\subsection{Slow Loops --- Maintaining Operating Point (D6, D7, D8, D9)}\label{slow-loops--maintaining-operating-point-d6-d7-d8-d9}


\begin{itemize}
\tightlist
\item
  \textbf{Tuner Loop} (\(\sim 0.01\)--\(1\) Hz): Adjusts cavity mechanical tuner to maintain \(\psi_\text{opt}\) (Eq. 2.6), compensating cavity thermal drift (D8) and microphonics (D7). Also re-converges to the new equilibrium after a ring-circumference-driven RF frequency update (D9).
\item
  \textbf{HVPS Loop} (\(\sim 1\) Hz): Adjusts klystron cathode voltage to maintain \(\sim 10\%\) below saturation.
\item
  \textbf{DAC Loop} (\(\sim 0.1\) Hz): Adjusts I/Q modulator baseline to maintain \(V_\text{gap}\) setpoint.
\end{itemize}

\subsection{Ripple Loop --- HVPS Harmonic Rejection (D5)}\label{ripple-loop--hvps-harmonic-rejection-d5}


Measures \textbf{klystron forward phase} (not cavity probe), avoiding the \(35.5\) kHz cavity BW limitation. DSP at \(\sim 23\) kHz sample rate tracks harmonics of 60 Hz.

\subsection{Multi-Loop Stability: The Bandwidth Separation Principle}\label{multi-loop-stability-the-bandwidth-separation-principle}
...
\begin{quote}
\textbf{Sources}: {[}R14{]}; {[}R15{]}; Åström \& Murray, \emph{Feedback Systems}, Ch. 12.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Loop-by-Loop Transfer Functions and Design}\label{loop-by-loop-transfer-functions-and-design}
...
\subsection{Loop Overview and Implementation Summary}\label{loop-overview-and-implementation-summary}
...
\bottomrule\noalign{}
\endlastfoot
Direct & 6.1 & Analog hardware (VXI RFP module) & Continuous & Lead-lag + PI & \textasciitilde800 kHz & Active \\
Comb & 6.2 & Digital hardware (VXI CFM, IIR + FIR) & \textasciitilde10 MHz & IIR comb + FIR equalizer & \textasciitilde2 MHz span & Not used \\
LFB Woofer & 6.3 & Digital hardware (GVF module, TAXI fiber) & \textasciitilde10 MHz & External injection & \textasciitilde1 MHz & Not used \\
Ripple & 6.4 & Digital hardware (AT\&T DSP1610) & 23 kHz & Adaptive harmonic estimator & \textasciitilde300 Hz & Active \\
Gap FF & 6.5 & Digital hardware (GVF module DSP) & \textasciitilde10 MHz & Feed-forward & \textasciitilde100 Hz & Not used \\
HVPS & 6.6 & Real-time software (EPICS SNL) & \textasciitilde1 Hz & Proportional + deadband & \textasciitilde1 Hz & Active \\
Tuner & 6.7 & Real-time software (EPICS SNL) & \textasciitilde1 Hz & Bang-bang & \textasciitilde0.01--1 Hz & Active \\
DAC & 6.8 & Real-time software (EPICS SNL) & Event / 10 s & Proportional + deadband & \textasciitilde0.1 Hz & Active \\
Gain Tracking & 6.9 & Supervisory software (EPICS CA) & \textasciitilde2 Hz & Ratio maintenance & \textasciitilde0.5 Hz & Active \\
\end{longtable}
...
\item
  \textbf{Real-time software} (\textasciitilde1 Hz): HVPS, tuner, and DAC loops --- EPICS programs managing slow plant dynamics (D6--D8).
\item
  \textbf{Supervisory software} (\textasciitilde0.1--2 Hz): Gain tracking --- EPICS Channel Access for calibration maintenance.
\end{enumerate}

\subsection{Direct Loop --- Wideband RF Field Regulation}\label{direct-loop--wideband-rf-field-regulation}
...
\textbf{Open-loop transfer function} (restating Eq. 5.1 with component forms from §5.2):
...
\begin{quote}
\textbf{Sources}: {[}R15{]} Corredoura, Figs. 3, 5; {[}R14{]} Schwarz; {[}R11{]} Gamp.
\end{quote}

\subsection{Comb Loop --- Narrowband Enhancement at Revolution Harmonics}\label{comb-loop--narrowband-enhancement-at-revolution-harmonics}
...
\textbf{IIR comb transfer function} (restating Eq. 5.4 with engineering context):
...
\begin{quote}
\textbf{Sources}: {[}R15{]} Corredoura, SLAC-PUB-8498, Figs. 4--6; {[}R14{]} Schwarz.
\end{quote}

\subsection{LFB Woofer --- Longitudinal Feedback via RF Modulation}\label{lfb-woofer--longitudinal-feedback-via-rf-modulation}
...
\begin{quote}
\textbf{Sources}: {[}R15{]} Corredoura, SLAC-PUB-8498; {[}R16{]} Fox et al., LFB system design; {[}R14{]} Schwarz.
\end{quote}

\subsection{Ripple Loop --- HVPS Harmonic Rejection}\label{ripple-loop--hvps-harmonic-rejection}
...
\textbf{Harmonic estimator algorithm} (from \texttt{ripple.s,v} {[}R36{]}): The DSP implements an adaptive harmonic tracker --- for each harmonic \(k\):
...
\begin{quote}
\textbf{Sources}: {[}R36{]} \texttt{ripple.s,v}; {[}R20t{]}; {[}R14{]} Schwarz §5.
\end{quote}

\subsection{Gap Feedforward Loop}\label{gap-feedforward-loop}
...
\subsection{HVPS Loop --- Klystron Operating Point Regulation}\label{hvps-loop--klystron-operating-point-regulation}
...
\textbf{Signal flow}: Klystron forward power monitor (or station gap voltage) → EPICS SNL (\texttt{rf\_\allowbreak{}hvps\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v}) → PLC HVPS voltage setpoint → Enerpro SCR firing angle → cathode voltage \(V_k\).
...
\begin{quote}
\textbf{Sources}: {[}R14{]}; \texttt{spear-\allowbreak{}rf-\allowbreak{}code-\allowbreak{}legacy/\allowbreak{}rfApp/\allowbreak{}src/\allowbreak{}seq/\allowbreak{}rf\_\allowbreak{}hvps\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v}.
\end{quote}

\subsection{Tuner Loop --- Cavity Resonant Frequency Tracking}\label{tuner-loop--cavity-resonant-frequency-tracking}


\textbf{Purpose}: Adjusts the mechanical cavity tuner to maintain the optimum detuning angle \(\psi_\text{opt}\) (Eq. 2.5a), compensating for beam loadng (main factor), cavity thermal drift (D8) and microphonics (D7). Also re-converges to the new equilibrium following a ring-circumference-driven shift in the RF operating frequency (D9).
...
\textbf{Control law} (from \texttt{rf\_\allowbreak{}tuner\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v} {[}R34{]}):
...
Motor controller & \textbf{AB 1746-HSTP1} (in service). Galil DMC-4143 Rev 1.3h is \textbf{in development, not installed} --- test moves were made Aug 2025; it goes online when the LLRF upgrade completes & --- \\
SPEAR3 status & \textbf{Active} & \\
\end{longtable}

\begin{quote}
\textbf{Sources}: {[}R14{]}; {[}R34{]} \texttt{rf\_\allowbreak{}tuner\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v}; {[}R35{]} Galil commissioning notes.
\end{quote}

\subsection{DAC Loop --- Outermost Amplitude Regulation}\label{dac-loop--outermost-amplitude-regulation}
...
\begin{quote}
\textbf{Sources}: \texttt{rf\_\allowbreak{}dac\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v}; \texttt{subIQ.c} (\texttt{subIQcounts}).
\end{quote}

\subsection{Gain Tracking Function}\label{gain-tracking-function}
...
\textbf{Implementation}: A slow EPICS calculation (\textasciitilde2 Hz) reads klystron forward and drive power monitors, computes current gain \(G_\text{kly} = P_\text{fwd}/P_\text{drive}\), and writes updated \(2\times 2\) matrix coefficients to the RFP Octal DAC. The \texttt{ConfDirect} calibration routine performs initial gain measurement and matrix setup.
...
\begin{quote}
\textbf{Sources}: {[}R15{]}; {[}R14{]}; \texttt{subSys.c} (\texttt{subSysDCcoeff}).
\end{quote}

\section{HVPS Plant Model and Dynamics}\label{hvps-plant-model-and-dynamics}


\subsection{Power Supply Architecture}\label{power-supply-architecture}
...
\subsection{HVPS Electrical Parameters}\label{hvps-electrical-parameters}
...
\begin{quote}
\textbf{Sources}: {[}R21{]} Cassel \& Nguyen, SLAC-PUB-7591; {[}R22{]} PS-341-360-01-R2.
\end{quote}

\subsection{SCR Phase Control Dynamics}\label{scr-phase-control-dynamics}
...
\begin{quote}
\textbf{Sources}: {[}R25{]} Bourbeau, IEEE 1983; {[}R26{]} Enerpro manual.
\end{quote}

\subsection{PLC Voltage Regulation Loop}\label{plc-voltage-regulation-loop}
...
\begin{quote}
\textbf{Sources}: {[}R27{]} \texttt{CasselPLCCode.\allowbreak{}pdf}; {[}R23{]} \texttt{hvps/\allowbreak{}simulation/\allowbreak{}hvps\_\allowbreak{}sim/\allowbreak{}config.\allowbreak{}py}.
\end{quote}

\subsection{Klystron Arc Protection}\label{klystron-arc-protection}
...
\subsection{HVPS Dynamic Response Summary}\label{hvps-dynamic-response-summary}
...
\section{Tuner Mechanics and Resonant Frequency Control}\label{tuner-mechanics-and-resonant-frequency-control}


\subsection{Cavity Tuner Physical Description}\label{cavity-tuner-physical-description}
...
\begin{quote}
\textbf{Sources}: {[}R29{]}--{[}R33{]} Tuner documentation in \texttt{llrf/tuners/}.
\end{quote}

\subsection{Tuning Physics}\label{tuning-physics}
...
\subsection{Tuner Control Loop}\label{tuner-control-loop}
...
where \(\psi_\text{target}\) is the target detuning angle (Eq. 2.6). Implemented in \texttt{rf\_\allowbreak{}tuner\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v} (EPICS SNL). Bandwidth \(\sim 0.01\)--\(1\) Hz.
...
\section{Performance Requirements and Verification}\label{performance-requirements-and-verification}


\subsection{Disturbance Rejection Summary}\label{disturbance-rejection-summary}
...
\subsection{LLRF9 Commissioning Results}\label{llrf9-commissioning-results}
...
\subsection{Performance Margins}\label{performance-margins}
...
Calibration establishes the numerical relationships between raw hardware signals (ADC counts, DAC counts, detector voltages) and physical quantities (kV, kW, degrees, Hz). It is essential for closing all feedback loops accurately. This section provides a high-level summary; detailed calibration procedures and data tables will be documented in \textbf{Doc D (Operational Data Catalog)}.

\subsubsection{Calibration Sequence Overview}\label{calibration-sequence-overview}


The master calibration is executed by \texttt{rf\_calib.st,v} (3,345 lines, 27 states). It runs as an EPICS SNL program and requires the station to be OFF (no RF power). The sequence progresses through the following phases:
...
\subsubsection{Drive Amplitude Calibration}\label{drive-amplitude-calibration}
...
\subsubsection{Power Measurement Calibration}\label{power-measurement-calibration}
...
\subsubsection{Frequency and Detuning Calibration}\label{frequency-and-detuning-calibration}
...
\subsubsection{Tuner Mode DAC Calibration}\label{tuner-mode-dac-calibration}


Data file: \texttt{tuneModeDacCalibration.\allowbreak{}xlsx}. Establishes DAC counts-to-drive-power for the \texttt{STATION\_TUNE} state (low-power cavity conditioning). Separate from operational drive calibration (§9.4.2) because the operating point differs.

\subsubsection{Signal Routing and Patch Panel}\label{signal-routing-and-patch-panel}


Data file: \texttt{b132R11PatchPanel.\allowbreak{}xlsx}. Documents physical signal routing in Building 132 R11: cable lengths, attenuation values, connector types, and signal naming. Essential for verifying the delay budget (§2.3) and signal levels.

\begin{quote}
\textbf{Upgrade note}: Many calibrations will need adaptation for the LLRF9 system (different ADC/DAC ranges, signal levels, processing gains). The polynomial frequency model (Eq. 6.7c) and combiner matrix calibration are expected to transfer; drive and power detector calibrations require new measurements.
\end{quote}

\begin{quote}
\textbf{Sources}: {[}R38{]}; {[}R39{]}; \texttt{rf\_calib.st,v}; \texttt{subIQ.c} (§2.2 of {[}R20u{]}).
\end{quote}
...
\section{SPEAR3 RF System Parameter Table}\label{spear3-rf-system-parameter-table}


\subsection{A.1 Storage Ring Parameters}\label{a1-storage-ring-parameters}
...
Total RF voltage (operational) & \(V_\text{RF}\) & \textasciitilde2.85 & MV & {[}R4{]} {[}R5{]} \\
Synchronous phase & \(\phi_s\) & 69.0 & deg & Eq. 2.4a (\(\cos\) convention) \\
Synchrotron tune & \(\nu_s\) & \textasciitilde0.008 & --- & Eq. 2.4b; {[}R1{]} \\
Synchrotron frequency & \(f_s\) & \textasciitilde10.1 & kHz & Eq. 2.4b \\
Long. radiation damping time & \(\tau_s\) & 2.87 & ms & {[}R1{]} \\
Trans. radiation damping time & \(\tau_{x,y}\) & 4.24, 5.14 & ms & {[}R1{]} \\
\end{longtable}

\subsection{A.2 Cavity Parameters (Per Cavity)}\label{a2-cavity-parameters-per-cavity}
...
Operational gap voltage & \(V_\text{gap}\) & \textasciitilde712 & kV & {[}R5{]} \\
...
Cavity wall losses & \(P_\text{wall}\) & \textasciitilde68.1 & kW/cav & Eq. 2.7a \\
Beam power per cavity & \(P_\text{beam}/n_\text{cav}\) & \textasciitilde127.5 & kW/cav & Eq. 2.7b \\
Generator power per cavity & \(P_\text{gen/cav}\) & \textasciitilde196 & kW & Eq. 2.7c \\
Total RF power (4 cav) & \(P_\text{gen,tot}\) & \textasciitilde782 & kW & \(4 \times P_\text{gen/cav}\) \\
\end{longtable}

\subsection{A.3 Klystron Parameters}\label{a3-klystron-parameters}
...
\subsection{A.4 Feedback Loop Parameters}\label{a4-feedback-loop-parameters}
...
\section{Source Document Reference Index}\label{source-document-reference-index}


\subsection{B.1 Published Papers}\label{b1-published-papers}
...
\subsection{B.2 Textbooks}\label{b2-textbooks}
...
\bottomrule\noalign{}
\endlastfoot
{[}R3{]} & SSRL SPEAR Storage Ring Parameters, \url{https://www-ssrl.slac.stanford.edu/accphy/spear_parameters.html} (last updated 26 Apr 2002, H.-D. Nuhn) \\
{[}R10{]} & Wiedemann, H., \emph{Particle Accelerator Physics}, 4th ed., Springer, 2015 \\
{[}R18{]} & Analog Devices AD834 datasheet \\
\end{longtable}

\subsection{B.3 Repository Documents}\label{b3-repository-documents}
...
\bottomrule\noalign{}
\endlastfoot
{[}R4{]} & LLRF9 Commissioning Tests & \texttt{llrf/\allowbreak{}tests/\allowbreak{}llrf9Tests.\allowbreak{}pdf} \\
{[}R5{]} & System Design Report & \texttt{Designs/\allowbreak{}tex/\allowbreak{}doc0-\allowbreak{}system-\allowbreak{}design/\allowbreak{}0\_\allowbreak{}system\_\allowbreak{}design\_\allowbreak{}report.\allowbreak{}pdf} \\
{[}R6{]} & RF System Description (Schwarz) & \texttt{llrf/\allowbreak{}documentation/\allowbreak{}legacyArchitecture/\allowbreak{}ps3403305100.\allowbreak{}pdf} \\
{[}R14{]} & Feedback Loop Description (Schwarz) & \texttt{llrf/\allowbreak{}documentation/\allowbreak{}legacyArchitecture/\allowbreak{}feedbackLoopDescriptionps3403305200.\allowbreak{}pdf} \\
{[}R19{]} & Comprehensive FBK Loops Description & \fpath{llrf/documentation/legacyArchitecture/feedbackLoopDescriptionps3403305200.pdf} \\
{[}R20t{]} & DSP Firmware Analysis & \texttt{spear-\allowbreak{}rf-\allowbreak{}code-\allowbreak{}legacy/\allowbreak{}codeReviewTechnicalNotes/\allowbreak{}04-\allowbreak{}dsp-\allowbreak{}firmware.\allowbreak{}md} \\
{[}R21t{]} & Cassel \& Nguyen, PAC 1997 --- SLAC-PUB-7591 & \fpath{hvps/architecture/originalDocuments/slac-pub-7591.pdf} \\
{[}R22{]} & HVPS Technical Spec & \texttt{hvps/\allowbreak{}architecture/\allowbreak{}originalDocuments/\allowbreak{}ps3413600102.\allowbreak{}pdf} \\
{[}R23{]} & HVPS Simulation Config & \texttt{hvps/\allowbreak{}simulation/\allowbreak{}hvps\_\allowbreak{}sim/\allowbreak{}config.\allowbreak{}py} \\
{[}R26{]} & Enerpro FCOG1200 Manual & \texttt{hvps/\allowbreak{}controls/\allowbreak{}enerpro/\allowbreak{}enerproDocuments/} \\
{[}R27{]} & Cassel PLC Code & \texttt{hvps/\allowbreak{}documentation/\allowbreak{}plc/\allowbreak{}CasselPLCCode.\allowbreak{}pdf} \\
{[}R28{]} & Cavity Tuner Inspection Report, 13 June 2023 & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}cavityTunerInspections20230613.\allowbreak{}docx} \\
{[}R29{]} & Tuner mechanical design paper, PAC 1997 & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}tunerDesignPac97HDS8P030.\allowbreak{}PDF} \\
{[}R30{]} & Superior Electric SLO-SYN stepper motor data & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}SLO-\allowbreak{}SYN.\allowbreak{}pdf} \\
{[}R31{]} & SLO-SYN MD808 Stepper Drive Manual & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}SLO-\allowbreak{}SYN\_\allowbreak{}MD808\_\allowbreak{}Stepper\_\allowbreak{}Drive\_\allowbreak{}Manual.\allowbreak{}pdf} \\
{[}R32{]} & SLO-SYN SS2000MD4M Step Drive / Translator Manual & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}SLO-\allowbreak{}SYN\_\allowbreak{}SS2000MD4M\_\allowbreak{}Step\_\allowbreak{}Drive\_\allowbreak{}Translator\_\allowbreak{}Manual.\allowbreak{}pdf} \\
{[}R33{]} & Superior Electric stepper catalogue (legacy) & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}Old\ Stepper\ Catalog\_\allowbreak{}Superior\ Electric\_\allowbreak{}0.\allowbreak{}pdf} \\
{[}R34{]} & Tuner Loop Source Code & \texttt{spear-\allowbreak{}rf-\allowbreak{}code-\allowbreak{}legacy/\allowbreak{}rfApp/\allowbreak{}src/\allowbreak{}seq/\allowbreak{}rf\_\allowbreak{}tuner\_\allowbreak{}loop.\allowbreak{}st,\allowbreak{}v} \\
{[}R35{]} & Galil Commissioning Notes & \texttt{llrf/\allowbreak{}tuners/\allowbreak{}galil/\allowbreak{}GalilCommissioning.\allowbreak{}docx} \\
{[}R36{]} & DSP Ripple Firmware & \texttt{spear-\allowbreak{}rf-\allowbreak{}code-\allowbreak{}legacy/\allowbreak{}rfApp/\allowbreak{}src/\allowbreak{}dsp/\allowbreak{}rfpDsp/\allowbreak{}ripple.\allowbreak{}s,\allowbreak{}v} \\
{[}R38{]} & RF Calibration Data & \texttt{llrf/\allowbreak{}calibrations/\allowbreak{}*.\allowbreak{}xlsx} \\
{[}R39{]} & RF Document Index & \texttt{llrf/\allowbreak{}documentation/\allowbreak{}RfSystemDocumentIndexR3.\allowbreak{}xlsx} \\
\end{longtable}

\subsection{B.4 External Web References}\label{b4-external-web-references}
...
\bottomrule\noalign{}
\endlastfoot
{[}W1{]} & \url{https://inspirehep.net/files/945e7ff73cc428af4c018fd1bdb6afa7} & McIntosh EPAC04 \\
{[}W2{]} & \url{https://www.osti.gov/biblio/839730} & OSTI SLAC-PUB-10983 \\
{[}W3{]} & \url{https://export.arxiv.org/pdf/1112.3203v1.pdf} & Gamp CAS 2011 \\
{[}W4{]} & \url{https://www.dimtel.com/products/llrf9} & Dimtel LLRF9 \\
{[}W5{]} & \url{https://proceedings.jacow.org/p85/PDF/PAC1985_1852.PDF} & Boussard PAC85 \\
{[}W7{]} & \url{https://inspirehep.net/files/dbbd3f9a808792f9901894060a879b5f} & Chang et al. NSRRC \\
{[}W8{]} & \url{https://www-ssrl.slac.stanford.edu/accphy/spear_parameters.html} & SSRL SPEAR Storage Ring Parameters {[}R3{]} \\
\end{longtable}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Symbol and Notation Conventions}\label{symbol-and-notation-conventions}


\subsection{C.1 Cavity and Beam Parameters}\label{c1-cavity-and-beam-parameters}
...
\subsection{C.2 Klystron and Drive Parameters}\label{c2-klystron-and-drive-parameters}
...
\subsection{C.3 Loop Transfer Functions and Compensator Parameters}\label{c3-loop-transfer-functions-and-compensator-parameters}
...
\subsection{C.4 Comb Filter and Group Delay Equalization Parameters}\label{c4-comb-filter-and-group-delay-equalization-parameters}
...
\subsection{C.4a LFB Woofer Parameters}\label{c4a-lfb-woofer-parameters}
...
\subsection{C.5 Ripple Loop Parameters}\label{c5-ripple-loop-parameters}
...
\subsection{C.6 Slow Loop Parameters}\label{c6-slow-loop-parameters}
...
\subsection{C.7 Calibration Parameters}\label{c7-calibration-parameters}
...
\subsection{C.8 I/Q Signal Processing}\label{c8-iq-signal-processing}
...
\subsection{C.9 Conventions}\label{c9-conventions}
...
\item
  \textbf{Bold symbols}: Vectors (e.g., \(\vec{V}_\text{ref}\), \(\vec{E}\)).
\item
  \textbf{Hat notation}: Estimated quantities (e.g., \(\hat{A}_k\)).
\item
  \textbf{Reference tags}: {[}Rn{]} = numbered reference, {[}Rnt{]} = transcription, {[}Wn{]} = web, {[}Dn{]} = disturbance (§4).
\item
  \textbf{Equation numbering}: (Eq. M.N) where M = section number, N = sequence within section. Lettered variants (a, b, c) denote closely related sub-equations.
\end{enumerate}
...
\item
  Definitive version: \fpath{Designs/tex/docP-rf-physics/P\_rf\_physics\_and\_plant.pdf} --- this document.
\end{itemize}
