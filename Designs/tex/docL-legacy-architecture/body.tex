...
\begin{fitpicture}\input{tikz/fig-control-hierarchy}\end{fitpicture}
...
\begin{fitpicture}\input{tikz/fig-system-overview}\end{fitpicture}
...
\begin{keynote}
\textbf{Cross-reference}: Doc~P \R{55} is the authoritative source for beam and cavity physics parameters and their derivations. Where this document and Doc P differ, Doc P governs for physics quantities and Doc L governs for as-installed hardware.
\end{keynote}
...
Output Current (typical operating) & ≈ \qtyrange{19}{22}{\ampere} & 22.0 A at full 500 mA operation \R{56}; 19.4 A in June 2020 (§4.3) \\
...
\begin{keynote}
A further bit, \texttt{AIM\_V\_RSTBMABT} (bit 7), resets the beam-abort latch. Doc~I §3.6 tabulates a different set of six (it includes \texttt{Forced\_Fault} and relegates \texttt{Filament\_Timeout} to a note); the grouping above follows the original Dusatko text.
\end{keynote}
...
\begin{keynote}
\textbf{Sources}: \R{53} §2.8; \R{16} (fault file handling code); \fpath{rfApp/src/db/p2RfAimDef.h,v} (register map, drawing 340-307); \R{20} (AIM status monitoring). For complete AIM/Fast IC architecture and signal details, see Doc~I §2--§3.
\end{keynote}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-comms-tree}\end{fitpicture}
...
\begin{keynote}
\textbf{Note}: Doc~I §5.7 and §6.4 carry the same assignment.
\end{keynote}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-vxi-crate}\end{fitpicture}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-clock-io}\end{fitpicture}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-clock-arch}\end{fitpicture}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-waveguide}\end{fitpicture}
...
\begin{keynote}
Full analysis of the tuner control loop \textbackslash u2014 inner phase loop, outer (disabled) load-angle loop, calibration polynomial, and failure modes \textbackslash u2014 is in Doc~T \R{57}. See also §17.
\end{keynote}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-hvps-chain}\end{fitpicture}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-regulation-loop}\end{fitpicture}
...
This part of Doc L summarizes the three protection subsystems (PPS, RF MPS, and Fast IC / interlock chain). For the complete interlock architecture --- including full signal flow diagrams, per-actor input/output tables, fault timeline examples, compliance analysis, and detailed fault data access procedures --- see Doc~I \R{73}.
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-pps-chains}\end{fitpicture}
...
\begin{figure}[tbp]\centering
\begin{fitpicture}\input{tikz/fig-pps-readback}\end{fitpicture}
...
Chain 2\textquotesingle s spring-return (fail-safe toward grounded) mitigates total PPS loss, but a PLC output staying energized despite PPS removal is not protected against by any hardware means. This is the primary driver for the planned upgrade to direct PPS control through the Interface Chassis. See \fpath{pps/HoffmanBoxPPSWiring.docx} for the upgrade architecture.

For the complete signal-chain details, compliance assessment, and all relay/wiring details, see Doc~I §6.8.

\begin{keynote}
\textbf{Sources}: \R{25}; \fpath{pps/HoffmanBoxPPSWiring.docx}.
\end{keynote}
...
When any protection condition is triggered, the RF MPS PLC relay simultaneously drives three protective outputs (the "three-path architecture"; see Doc~I §5.3--§5.4):
...
\begin{keynote}
\textbf{Sources}: MPS wiring diagrams \R{21}; \R{5} §7; Doc~I §5.
\end{keynote}
...
Four data sources capture fault event information at different time scales. Full access procedures and a step-by-step analysis guide are provided in Doc~I Part X (§10.1--10.6). Summary:
...
\begin{keynote}
\textbf{Sources}: Doc~I Part X (§10.1--10.6); §18.2; \fpath{rfApp/Db/rf_hvps.db} (HVPS scalar PV definitions).
\end{keynote}
...
§16 SNL software & C & Line counts and state counts from \fpath{spear-rf-code-legacy/codeReviewTechnicalNotes/05-snl-state-machines.md}; only RCS \texttt{,v} archives are present in the repository, so working-file line counts could not be recounted directly \\
...
\RDEF{5} & SPEAR3 LLRF Upgrade System Physical Design Report, \textbf{R2} (28 April 2026) --- the authoritative upgrade reference & \fpath{Designs/docx/SPEAR3_LLRF_PDR_R2.docx}; typeset R1 at \fpath{Designs/tex/doc0-system-design/0_system_design_report.pdf} \\
...
\RDEF{55} & SPEAR3 RF Physics and Plant (Doc P) --- authoritative for beam and cavity physics parameters & \fpath{Designs/tex/docP-rf-physics/P\_rf\_physics\_and\_plant.pdf} \\
...
\RDEF{57} & SPEAR3 Legacy LLRF Tuner Control System Technical Analysis (Doc T) & \fpath{Designs/tex/docT-tuner-control/T\_tuner\_control\_analysis.pdf} \\
...
\RDEF{73} & \textbf{SPEAR3 RF Station Legacy System Interlock Architecture (Doc I)} --- companion document; authoritative for the complete legacy interlock signal flow, per-actor input/output tables, fault timelines and fault data access procedures & \fpath{Designs/tex/docI-interlock-architecture/I\_interlock\_architecture.pdf} \\
\end{longtable}
...
\item
  The definitive version is the LaTeX source \fpath{Designs/tex/docL-legacy-architecture/L_legacy_system_architecture.tex} in the \texttt{spearlegacyLLRF} repository; the PDF is built from it by \fpath{Designs/tex/build.ps1}.
\item
